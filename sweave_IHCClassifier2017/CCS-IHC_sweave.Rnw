...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
\section{Running the Automated Classification Pipeline}
\subsection{Patient samples: Marker staining}

FFPE patient material was used to construct TMAs in this study, with diameter 0.6mm - 1mm. Depending on quantaties of material available, 1-3 cores were obtained from each patient. Below is a detailed protocol to conduct immunohistochemistry staining:

Slides were created by transversely sectioning at 4$\mu$m. The paraffin-embedded slides were deparaffinated with xylene and rehydrated through a series of ethanol concentrations. Endogenous peroxidase activity was blocked by incubating in 0.3\% H2O2 in PBS at room temperature for half an hour, where after antigen retrieval was achieved by heating the slides in a citrate buffer, pH 6.0, for 20 minutes; followed by cooling in the same buffer for 10 minutes. The slides were incubated with a diluted primary antibody for one hour at room temperature or overnight at 4°C.

\begin{table}[!ht]
\centering
  \begin{tabular}{lccc}
    anti-CDX2  & 1:200 & Novus Biologicals & NB100-2136 \\       
    anti-HTR2B  & 1:75 & Sigma & HPA012867 \\ 
    anti-FRMD6 & 1:500 & Sigma & HPA001297 \\
    anti-ZEB1 & 1:500 & Sigma & HPA027524 \\
    anti-cytokeratin (AE1/AE3) & 1:500 &  Thermo Scientific & \\
  \end{tabular}
  \caption{List of IHC markers used}
  \label{tab:1}
\end{table}

Incubation with an undiluted secondary antibody (anti-mouse-HRP  or anti-rabbit-HRP (Powervision)) for 30 minutes followed.
Rinsing between steps was performed with PBS. The slides were developed with diaminobenzidine and counterstained with Mayers’ haematoxylin. Here after the slides were dehydrated and mounted with cover slips. Digital images of TMAs were then acquired and segmented into individual cores (e.g. using Aperio Slide Scanner).

The original references for TMA construction and MSI testing are: CAIRO series: \citep{Koopman2009a}, \citep{Hendriks2003}; LUMC series:\citep{Zeestraten2014}, \citep{Goossens2014}; AMC: \citep{Melo2013}

\subsection{Running Image Analysis in MATLAB}
...
Below, we construct a MATLAB script to analyse an image and observe the outputs from this: A copy of this script is available in the github repository \url{github.com/trinhan/crc-ihc-classification}.
...
The resultant information required for the classification pipeline is a table for each stain containing:

\begin{enumerate}
\item The image ID. This should contain information on the location of the image on the TMA (eg. R1C2 indicates the image is in location row 1, column 2) 
\item TMA core area (from MATLAB)
\item Fraction of the image containing a core (from MATLAB)
\item Stain Area (from MATLAB)
\item Stain Intensity (from MATLAB)
\item Stain Area Fraction (from MATLAB)
\end{enumerate}

\begin{figure}[!ht]
\includegraphics[width=1\textwidth]{Table_Format.png}
\caption{Example of table required for classification}
\end{figure}

One file/table per slide is permitted.

\subsection{Organising data from Image-Analysis for classification in R}

The following steps describes the formating of data required for input, as shown in Figure 2.

\begin{figure}[!ht]
\includegraphics[width=1\textwidth]{info_overview_pipeline.pdf}
\caption{Information required for classification system: A. TMA maps. Multiple maps are permitted B. 5x marker table from image analysis. C. Table identifying TMA slides with maps D. Table containing MSI status(optional)}
\end{figure}

Briefly, the main files required are:

\begin{itemize}
\item \textbf{A. TMA map} Links the layout of the TMA with patient information. Table size should be the same as that extracted from image analysis. Eg. if a 20x7 scores are segmented from a slide, the TMA map should have 20x7 entries. Replicates from the same patient are permitted: the IDs should not change (eg. for Patient 1, multiple entries of "1" is sufficient. Labelling of core number "1.1", "1.2" ... is not necessary). Empty cores can be designated with "x".
\item \textbf{B. Table of image information} For each TMA map, there should be 5 tables - one for each stain. Image number should indicate the row number prior to the column number (eg, R2C1 refers to row 2, column 1. Also permitted are underscores to separate the values eg. "CDX2\textunderscore 1 \textunderscore 2.csv", however, the former is suggested to avoid confusion). Additionally, each slide should have a 6 digit unique ID number. Filenames all the tables in B should contain this ID number (Eg. a suitable filename for CDX2 Map1 can be "CDX2\textunderscore 012345.csv"). Files without this ID number will be designated a value by alphabetical order.
\item \textbf{C. Table containing maps for each slide}  Table C should be named ("cohortname\textunderscore slide\textunderscore no.csv"), and contain a table of TMA maps and unique identifiers from B.
\item \textbf{D. List of samples and MSI status} 1 should indicate "MSI" and 0 should indicate "MSS" ("MSI" and "MSS" is also permited, but must be uppercase). This list is optional - these patients can be removed after the analysis. However, there may be an epithelial vs mesenchymal population weighting, so it is advised that these patients are removed beforehand.
\end{itemize}

\subsection{Running the automated classification}

The following commands are required for running the classification. A copy of the AMC-classifier is available on the git hub repository, as well as the script to run this classification:


\textbf{Step 1: } Firstly, the data needs to be arranged into two folders of interest: a folder containing the data of interest (in this case, this is the "Cairo1 set"), and a folder containing the maps of interest as wel as the table mapping  ("mapping Cairo1").
...
\texttt{ReadFromCSV} will generate a list of RData files - one for each TMA map combining all stain information. \texttt{LoadFiles} will concatenate these files into a large matrix containing information from all samples

\textbf{Step 2: } Next we preprocess the data.Load the clinical information in order to remove MSI+ samples from the cohort. We then preprocess the data to remove samples with missing cores: The entries for this will be the (a) \texttt{Cai1data} matrix defined in step 1, (b) the clinical information (column names indicating the relevant information need to be specified: i.e. the columns with the patient ids and msi status - here, designated as "ID" and "MSI"). (c) Additionally, as this is a `Test' set instead of a `Train' set, this will need to be specified. If a set is to be used as a "Train"ing set, part b must also include information on the class (d) a permitted image fraction range. This is dependent on how the cores are segmented and a full core should lie within the range of 0.3 to 0.6. Examine the ``ImFrac" values from image analysis to determine an optimal range. In this case, 0.1 to 0.8 is used as some of the cores occupy most of the image

<<eval=F>>==
Cai1data.edit=PreProcess(Cai1data, Cai1Clin[ , c("ID", "MSI")], 'Test', c(0.1, 0.8))
@

\textbf{Step 3: } Scale the information. We no longer need information on the TMA core size or fraction - this was used solely as a quality control step. These columns are removed using the \texttt{grep} function. Also, this data is then scaled using the \texttt{MedScake} function which performs median scaling
...
\textbf{Step 4:} Clasification. We will need to load the AMC-classifier, as well as the random forest library. Then use the \texttt{predict} function to obtain a prediction probability from each core. You can examine the object \texttt{C1\textunderscore R} to see the prediction probabilities. Finally, compress the scores. We will need a list of patient IDs (defined in step 2). There are 3 ways of performing the prediction: (1) any CMS4 is labelled as CMS4. (2) majority consensus (currently used) and (3) based on a combined prediction probability greater than a given threshold. Furthermore, a prection threshold is used to define CMS4 - this is currently set to 0.6 in a hope to avoid false positives. 

<<eval=F>>==
load("AMC_classifier.RData")
...
The AMC dataset was used previously \citep{Melo2013} as a discovery set to uncover three subtypes of CRC: CMS2/3, defined by chromosomal instability , CMS1 characterised by microsatellite instability (MSI) and CMS4 characterised by EMT. In this study, this set will be used as the training set to classify patients in the CAIRO set, CAIRO-2 set and the LUMC(Leiden) set.
...
Next, we want to remove patients with missing cores and MSI positive (or unknown) patients to produce a twoclass classification system (between CMS2/3 and CMS4).
...
Note that the training will be based on the GE labels: There are some patients which are MSI positive who are CMS4 by GE, which we are using as our 'ground truth', and thus included in the classifier training. These patients will be 'misclassified' as CMS1 by TMA-IHC.
...
Our CAIRO cohort will consist of 19 MSI (CMS1 patients), and 407 patients (with 411 cores) to be classified as CMS2/3 or CMS4.
...
In the CAIRO-2 set, we have 8 CMS1 patients and 332 patients to be classified as CMS2/3 or CMS4.
...
This set has a larger number of MSI patients (35), and 205 of these patients (with 605 cores) are to be classified as CMS2/3 vs CMS4.
...
# tempdata=tempdata[ ,-grep("KER_Brown.total.norm", colnames(tempdata))]
 ...
 \subsection{FIG S2: Correlation between CMS and pathologist scored staining}

 We can also look at the consistency between pathologist scoring and the CMS subtypes:
...
There is a tendency in CMS4 samples to have higher ZEB1 and CDX2 staining with P values from Chi-sq test less than 0.05, however, not all patient samples display this characteristic. We will need to consider the combination of stains to determine CMS2/3 from CMS4.
 
How consistent is inter-patholgoist scoring?

<<fig.height=6>>==
PathScorAMC=read.csv("path_scoring/AMC_3pathologists.csv", header=T)
#
PathScorAMC$CCS=AMCclin$Class[match(PathScorAMC$ID, toupper(AMCclin$ID))]

stains=c("CDX2", "HTR2B", "FRMD6", "ZEB1")
ICCv=rep(NA, 4)

for (i in 1:4){
  l1=data.matrix(PathScorAMC[ ,grep(stains[i], colnames(PathScorAMC))])
  ICCv[i]=ICC(l1, missing=F)$results[3, 2]
}

barplot(t(ICCv), beside=T, ylab="Consistency")
@

Does the other pathologist scoring show associations between the scores and 

<<fig.height=6, eval=F, echo=F>>==
par(mfrow=c(2,2))

#PathScorAMC=PathScorAMC[PathScorAMC$CCS!=2, ]

ContTable(table(PathScorAMC$CCS, PathScorAMC$CDX2KT),"CDX2 staining", T ,"CDX2 stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$HTR2BKT),"HTR2B staining", T ,"HTR2B stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$FRMD6KT),"FRMD6 staining", T ,"FRMD6 stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$ZEB1KT),"ZEB1 staining", T ,"ZEB1 stain")

ContTable(table(PathScorAMC$CCS, PathScorAMC$CDX2GO),"CDX2 staining", T ,"CDX2 stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$HTR2BGO),"HTR2B staining", T ,"HTR2B stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$FRMD6GO),"FRMD6 staining", T ,"FRMD6 stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$ZEB1GO),"ZEB1 staining", T ,"ZEB1 stain")

ContTable(table(PathScorAMC$CCS, PathScorAMC$CDX2MJ),"CDX2 staining", T ,"CDX2 stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$HTR2BMJ),"HTR2B staining", T ,"HTR2B stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$FRMD6MJ),"FRMD6 staining", T ,"FRMD6 stain")
ContTable(table(PathScorAMC$CCS, PathScorAMC$ZEB1MJ),"ZEB1 staining", T ,"ZEB1 stain")

@

\subsection{FIG S2: Morphological associations in the Dataset}

Check for associations between the differentiation state and CMS classification

<<fig.height=4>>==
par(mfrow=c(1,2))

# check associations between CMS and diff
ContTable(table(AMCclin$Class, AMCclin$DiffGrade),"CMS", T ,"DiffGrade")
# also CMS and diff stage
ContTable(table(AMCclin$DiffGrade[match(PathScorAMC$ID, toupper(AMCclin$ID))],PathScorAMC$CDX2MJ),"DiffGrade", T, "CDX2 stain")
# ZEB1 and diff?
ContTable(table(AMCclin$DiffGrade[match(PathScorAMC$ID, toupper(AMCclin$ID))],PathScorAMC$ZEB1MJ),"DiffGrade", T, "ZEB1 stain")
@

\subsection{FIG S2: Comparison with Legget and Whitehall}

Compare the CMS classification with that conducted by Legget and Whitehall on genomic markers

<<fig.height=4>>==
WLtable=AMCclin[ ,c("MSIStatus", "Cimpmethylight", "Ras", "BRAF", "Class", "Met", "stage", "Sex")]

# any MSI goes into the same group
WLtable$MSIStatus[WLtable$MSIStatus=="MSI-L"]="MSI-H"
WLtable$MSIStatus=factor(WLtable$MSIStatus)

# reorder the CIMP status
WLtable$Cimpmethylight=factor(WLtable$Cimpmethylight, levels(WLtable$Cimpmethylight)[2:1])

# define the subtypes from Legget and Whitehall
Norm=which(WLtable$MSIStatus=="MSS" & WLtable$Ras=="WT" & WLtable$BRAF=="WT" & WLtable$Cimpmethylight=="CIMP-")
Alt=which(WLtable$MSIStatus=="MSS" & WLtable$Ras=="Mut" & WLtable$BRAF=="WT" & WLtable$Cimpmethylight=="CIMP-")
Serr=which(WLtable$Ras=="WT" & WLtable$BRAF=="MUT" & WLtable$Cimpmethylight=="CIMP+")

WLtable$WLgroup="una"
WLtable$WLgroup[Norm]="trad"
WLtable$WLgroup[Alt]="alt"
WLtable$WLgroup[Serr]="serr"

CCSclass=factor(WLtable$Class)
levels(CCSclass) = c("skyblue", "palegreen", "darkblue")

WLclass=factor(WLtable$WLgroup)
levels(WLclass)=brewer.pal(4, "Set1")

levels(WLtable$Met)=c("white","black")

T1=heatmap.plus(t(data.matrix(WLtable[ ,1:4])), col=c("darkblue","grey"), trace="none", 
             ColSideColors = cbind(as.character(CCSclass), as.character(WLclass), as.character(WLtable$Met)), scale="none")

par(mfrow=c(1,2))

ContTable(table(WLtable$Class, WLtable$WLgroup), "comparison diff", chisqtest = T)

summary(WLtable)

s1=Surv(as.numeric(AMCclin$timeMETRec2), as.numeric(AMCclin$Met))
plot(survfit(s1~WLgroup, data=WLtable), lwd=2, col=brewer.pal(4, "Set1"),
      main="AMC set", xlab="time (months)", ylab="Disease Free Survival")

WLtable$WLgroup=factor(WLtable$WLgroup, levels=c("trad", "una", "serr", "alt"))

summary(coxph(s1~WLgroup+Sex+stage, data=WLtable))
@
...
Using the predicted scores from gene expression, we select cores from the CMS2/3 and CMS4 labelled patients to train a random forest classifier. 

\subsubsection{Training the classifier}
Firstly, we will test whether the combination of cores per patient predicts the gene expression classification. This will take into the number of cores classified as CMS2/3 vs CMS4, alongside the probability attained from random forest
...
# Label Patients as CMS1 if MSIpositive  
MSIpos=toupper(AMCclin$ID[which(AMCclin$MSI=="MSI" & AMCclin$Class!=3)])
  # if number of CMS4 cores is greater than or equal to CSS1 cores, label as CSS3
...
# rename patients which are MSI & CMS4 to CMS1
...
matrix(summary(t1,times = seq(0, 60, 15))$n.risk, nrow=3, byrow=T,
       dimnames=list(c("CMS2/3", "CMS4", "CMS1"), c(seq(0, 60, 15))))
# check the coxph model:
res1=coxph(s1~stage+Sex+factor(x2), data=AMCclin)
summary(res1)
 @

Prognostic difference between CMS2/3 and CMS4 patients are observed here.
...
Can also do this by constructing decision trees using single features?

<<summary_error_bars,cache=T>>==
ErrRates=matrix(NA, ncol=29, nrow=100)

for (i in 1:28){
  for (j in 1:100){
Idx=sample(1:nrow(tdat), round(nrow(tdat)*2/3))
TrainSet=tdat[Idx,c(i,29) ]
TestSet=tdat[-Idx,c(i,29) ]
# Train the classifier
rf1 = randomForest(CCS ~ ., data=TrainSet, importance=TRUE, ntree=500)
# Make prediction
#rfpred =predict(rf1, TestSet, type="response")
#Acc=table(rfpred, TestSet[ ,ncol(TestSet)])
#Acc
ErrRates[j,i]=mean(rf1$err.rate[,1])
}
}
...
TestSet=tdat[-Idx, ]
rf1 = randomForest(CCS ~ ., data=TrainSet, importance=TRUE, ntree=500)
ErrRates[j,29]=mean(rf1$err.rate[,1])
}

colnames(ErrRates)=colnames(tdat)[1:29]

MeanVals=colMeans(ErrRates[ ,-29])

x1=order(MeanVals)

BplotCols=colnames(ErrRates)
repNames=cbind(c("FRMD6", "ZEB1", "HTR2B", "CDX2", "KER"), 
              c("yellow", "pink", "palegreen", "skyblue", "brown"))
 for (i in 1:5){
   BplotCols[grep(repNames[ i, 1], BplotCols)]=repNames[ i, 2]
...
boxplot(ErrRates[ ,c(  x1,29)], horizontal=T, col=c(BplotCols[x1], "red"), xlab="Error Rate", ylab="Stain", las=2)
@
\subsection{}
...
# table of numbers at risk
matrix(summary(t1,times = seq(0, 60, 15))$n.risk, nrow=3, byrow=T,
       dimnames=list(c("CMS2/3", "CMS1", "CMS4"), c(seq(0, 60, 15))))
@
...
# Number of CMS2/3 vs CMS4 cores:
summary(C1_R[ ,2]<=0.6)
...
matrix(c(l1[1:7], 0, l1[8:11]), nrow=3, byrow=T,
       dimnames=list(c("CMS2/3", "CMS1", "CMS4"), c(seq(0, 45, 15))))
 @
...
We see a significant difference between CMS2/3 and CMS4 patients

\subsubsection{Fig S3: Compare Treatment Arms for each CMS}

Given the subtype classification, does the treatment arm have an impact on survival of the CMS2/3 vs CMS4 subgroups?

 <<Fig4A, fig.height=4>>==
# Make the plots
par(mfrow=c(1,2))
plot(survfit(ss~treatment.Arm, data=Cai1Clin, subset=R2==1),
      col=c(1), lty=c(1:2),  main=" CMS2/3", lwd=2, ylab="Overall Survival", xlab="Time (Months)")
plot(survfit(ss~treatment.Arm, data=Cai1Clin, subset=R2==3),
      col=c(3),lty=c(1:2), main="CMS4", lwd=2, ylab="Overall Survival", xlab="Time (Months)")
t1=survfit(ss~R2+Geslacht, data=Cai1Clin, subset=R2!=2)
t1
# Table of numbers at risk: Note, need to insert a 0 as there are no CMS1 cases at risk at
matrix(summary(t1,times = seq(0, 45, 15))$n.risk, nrow=4, byrow=T,
       dimnames=list(c("ArmA_CMS2/3", "ArmB_CMS2/3", "ArmA_CMS4", "ArmB_CMS4"), 
                     c(seq(0, 45, 15))))
...
CMS2/3 patients display no difference with treatment arm; CMS4 display a mild benefit with treatment B compared to A (combination therapy)
...
It appears in both CMS2/3 and CMS4, patients are more likely to have a better response in Arm B

\subsection{Comparison with existing subtypes}

As CIMP is not available for this analysis, MSI+ will indicate a CIMP+ class

<<>>==
WLtable=Cai1Clin[ ,c("KRAS", "BRAF", "MSI", "R2", "Geslacht", "Age", "treatment.Arm")]

# define the subtypes from Legget and Whitehall
Norm=which(WLtable$MSI=="MSS" & WLtable$KRAS==0 & WLtable$BRAF==0)
Alt=which(WLtable$MSI=="MSS" & WLtable$KRAS==1 & WLtable$BRAF==0)
Serr=which(WLtable$KRAS==0 & WLtable$BRAF==1 & WLtable$MSI=="MSI")
...
CCSclass=factor(WLtable$R2)
levels(CCSclass) = c("skyblue", "palegreen", "darkblue")

WLclass=factor(WLtable$WLgroup)
levels(WLclass)=brewer.pal(4, "Set1")

WLtable$MSI=-as.numeric(WLtable$MSI)+2

x2=which(is.na(WLtable[ ,1:3]), arr.ind=T)

WLtable=WLtable[-x2[,1], ]

T1=heatmap.plus(t(data.matrix(WLtable[ ,c(1:3)])), col=c("grey", "darkblue"), trace="none", 
             ColSideColors = cbind(as.character(CCSclass[-x2[ ,1]]), as.character(WLclass[-x2[ ,1]])), scale="none")

par(mfrow=c(1,2))

ContTable(table(WLtable$R2, WLtable$WLgroup), "comparison diff", chisqtest = T)

s1=Surv(as.numeric(Cai1Clin$OS_MONTH), as.numeric(Cai1Clin$OS_event))
plot(survfit(s1[-x2[ ,1]]~WLgroup, data=WLtable), lwd=2, col=brewer.pal(4, "Set1"),
      main="AMC set", xlab="time (months)", ylab="Disease Free Survival")

WLtable$WLgroup=factor(WLtable$WLgroup, c("trad", "serr", "una", "alt"))
summary(coxph(s1[-x2[ ,1]]~WLgroup+Geslacht+Age+treatment.Arm, data=WLtable))
...
We see a better prognosis in the CMS2/3 patients compared to CMS4
...
summary(coxph(ss~AgeCat,data=Cai2Clin))
@
...
Cai2.Rcox=coxph(ss~factor(Sex)+factor(Arm)+factor(R2)+AgeCat, data=Cai2Clin) 
summary(Cai2.Rcox)
Cai2.Rcox=coxph(ss~factor(Sex)+factor(Arm)+factor(R2)+factor(R2)*factor(Arm)+AgeCat, data=Cai2Clin) 
summary(Cai2.Rcox)
@
...
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==0 & R2==1)),col=c(1), main="KRAS wt CMS2/3", lwd=2, lty=c(1:2))
legend("topright", c("ArmA", "ArmB"), col=1, lwd=2, lty=c(1, 2), cex=0.75)
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==0 & R2==3)),col=3, main="KRAS wt CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("ArmA", "ArmB"), col=3, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==1 & R2==1)),col=1, main="KRAS mut CMS2/3", lwd=2,lty=c(1, 2))
legend("topright", c("ArmA", "ArmB"), col=1, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==1 & R2==3)),col=3, main="KRAS mut CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("ArmA", "ArmB"), col=3, lwd=2, cex=0.75, lty=c(1:2))
...
Cai2.A1=coxph(ss~Arm+Sex+Age, data=Cai2Clin, subset=(BRAF_KRAS==0 & R2==1))
summary(Cai2.A1)
Cai2.A3=coxph(ss~Arm+Sex+Age, data=Cai2Clin, subset=(BRAF_KRAS==0 & R2==3))
summary(Cai2.A3)
Cai2.B1=coxph(ss~Arm+Sex+Age, data=Cai2Clin, subset=(BRAF_KRAS==1 & R2==1))
summary(Cai2.B1)
Cai2.B3=coxph(ss~Arm+Sex+Age, data=Cai2Clin, subset=(BRAF_KRAS==1 & R2==3))
summary(Cai2.B3)

t1=survfit(ss~Arm+BRAF_KRAS+R2, data=Cai2Clin, subset=(R2!=2))
t1
# Table of numbers at risk: note the numbers are Arm/KRASmut/CMS subtype
...
par(mfrow=c(2,2))
# Segregate according to KRAS or BRAF and Treatment Arm
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="cntl" & R2==1)),col=c(1), main="Cntl Arm CMS2/3", lwd=2, lty=c(1:2))
legend("topright", c("wt", "mut"), col=1, lwd=2, lty=c(1, 2), cex=0.75)
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="cntl" & R2==3)),col=3, main="Cntl Arm CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("wt", "mut"), col=3, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="wCetux"& R2==1)),col=1, main="Cetux CMS2/3", lwd=2,lty=c(1, 2))
legend("topright", c("wt", "mut"), col=1, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="wCetux" & R2==3)),col=3, main="Cetux CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("wt", "mut"), col=3, lwd=2, cex=0.75, lty=c(1:2))
title("Cairo2 treatment based", outer=TRUE)
mtext("Time (Months)", 1, 3, outer=TRUE)
mtext("Time (Months)", 2, 3, outer=TRUE)
...
# Harzard Ratios for wt BRAF/KRAS patients with different treatment arms:
Cai2.A1=coxph(ss~BRAF_KRAS+Sex+Age, data=Cai2Clin, subset=(Arm=="wCetux" & R2==1))
summary(Cai2.A1)
Cai2.A3=coxph(ss~BRAF_KRAS+Sex+Age, data=Cai2Clin, subset=(Arm=="wCetux" & R2==3))
summary(Cai2.A3)
Cai2.B1=coxph(ss~BRAF_KRAS+Sex+Age, data=Cai2Clin, subset=(Arm=="cntl" & R2==1))
summary(Cai2.B1)
Cai2.B3=coxph(ss~BRAF_KRAS+Sex+Age, data=Cai2Clin, subset=(Arm=="cntl" & R2==3))
summary(Cai2.B3)
@



We see a survival benefit in the CMS2/3 subtype with  cetuximab in the KRAS wt subgroup, but not in the CMS4 subtype.

Plot on the same plot the differences in CMS2/3 vs CMS4
<<fig.height=4>>==
par(mfrow=c(1,2))
plot(survfit(ss~R2+Arm, data=Cai2Clin, subset=(BRAF_KRAS==0 & R2!=2)),col=c(1,1, 3, 3), main="KRAS wt CMS2/3 vs CC3", lwd=2, lty=c(1:2), xlab="months", ylab="OS")
legend("topright", c("Arm1_CMS2/3wt", "Arm2_CMS2/3wt", "Arm1_CMS4wt", "Arm2_CMS4wt"), 
        lwd=2, cex=0.75, lty=c(1:2), col=c(1,1,3,3))
Cai2Clin$tIdx=Cai2Clin$BRAF_KRAS==0 & Cai2Clin$R2==1
plot(survfit(ss~tIdx+Arm, data=Cai2Clin, subset=(R2!=2)),col=c(5,5,1,1), main="KRAS/BRAF wt CMS2/3 vs All other patients", lwd=2, lty=c(1:2), xlab="months", ylab="OS")
legend("topright", c( "Arm1_other", "Arm2_other","Arm1_CMS2/3wt", "Arm2_CMS2/3wt"), 
       lwd=2, cex=0.75, lty=c(1:2), col=c(5,5,1,1))

## Determine the hazards ratios:
## Test CMS impact on Arm2 in wt patients
summary(coxph(ss~R2, data=Cai2Clin, subset=(BRAF_KRAS==0 & R2!=2 & Arm=="wCetux"))) 
## Test CMS2/3wt Arm2 with the Arm2 Other
...
<<>>==
WLtable=Cai2Clin[ ,c("KRAS_01", "BRAF", "MSI", "R2", "Age", "Arm", "Sex")]

# define the subtypes from Legget and Whitehall
Norm=which(WLtable$MSI=="MSS" & WLtable$KRAS_01==0 & WLtable$BRAF==0)
Alt=which(WLtable$MSI=="MSS" & WLtable$KRAS_01==1 & WLtable$BRAF==0)
Serr=which(WLtable$KRAS_01==0 & WLtable$BRAF==1 & WLtable$MSI=="MSI")
...
x2=which(is.na(WLtable[ ,c(1:4)]), arr.ind=T)

WLtable$MSI=-as.numeric(WLtable$MSI)+1

WLtable=WLtable[-x2[ ,1], ]

T1=heatmap.plus(t(data.matrix(WLtable[ ,1:3])), col=c("grey","darkblue"), trace="none", 
             ColSideColors = cbind(as.character(CCSclass[-x2[ ,1]]), as.character(WLclass[-x2[ ,1]])), scale="none")
...
s1=Surv(as.numeric(Cai2Clin$OS_month), as.numeric(Cai2Clin$OS_event))
plot(survfit(s1[-x2[ ,1]]~WLgroup, data=WLtable), lwd=2, col=brewer.pal(4, "Set1"),
      main="AMC set", xlab="time (months)", ylab="Disease Free Survival")

WLtable$WLgroup=factor(WLtable$WLgroup, c("trad", "serr", "una", "alt"))
summary(coxph(s1[-x2[ ,1]]~WLgroup+Sex+Age+Arm, data=WLtable))
...
# table per patient level: add in CMS1 patients
...
write.csv(C2_A2, file=sprintf("Cairo2_Classes_pathscore%s.csv", Sys.Date()))
@
...
# calculate P value
summary(coxph(ss~factor(Pcat)+Sex+Arm+Age, data=Cai2Clin))

par(mfrow=c(2,2))
## Reproduce BRAF/KRAS vs CMS
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==0 & Pcat==1)),col=c(1), main="KRAS wt CMS2/3", lwd=2, lty=c(1:2))
legend("topright", c("ArmA", "ArmB"), col=1, lwd=2, lty=c(1, 2), cex=0.75)
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==0 & Pcat==3)),col=3, main="KRAS wt CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("ArmA", "ArmB"), col=3, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==1 & Pcat==1)),col=1, main="KRAS mut CMS2/3", lwd=2,lty=c(1, 2))
legend("topright", c("ArmA", "ArmB"), col=1, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~Arm, data=Cai2Clin, subset=(BRAF_KRAS==1 & Pcat==3)),col=3, main="KRAS mut CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("ArmA", "ArmB"), col=3, lwd=2, cex=0.75, lty=c(1:2))

par(mfrow=c(2,2))
## Reproduce Arm vs CMS
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="cntl" & Pcat==1)),col=c(1), main="Cntl CMS2/3", lwd=2, lty=c(1:2))
legend("topright", c("wt", "mut"), col=1, lwd=2, lty=c(1, 2), cex=0.75)
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="cntl" & Pcat==3)),col=3, main="Cntl CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("wt", "mut"), col=3, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="wCetux" & Pcat==1)),col=1, main="cetux CMS2/3", lwd=2,lty=c(1, 2))
legend("topright", c("wt", "mut"), col=1, lwd=2, cex=0.75, lty=c(1:2))
plot(survfit(ss~BRAF_KRAS, data=Cai2Clin, subset=(Arm=="wCetux" & Pcat==3)),col=3, main="cetux CMS4", lwd=2,lty=c(1, 2))
legend("topright", c("wt", "mut"), col=3, lwd=2, cex=0.75, lty=c(1:2))
...
plot(survfit(ss~Pcat+Arm, data=Cai2Clin, subset=(BRAF_KRAS==0)), col=c(1,1,2,2,3,3),main="KRASwt CMS2/3 vs all KRAS wt", lwd=2,lty=1:2, xlab="months", ylab="OS")
legend("topright", c("Arm1_CMS2/3wt", "Arm2_CMS2/3wt", "Arm1_CMS4wt", "Arm2_CMS4wt"), 
        lwd=2, cex=0.75, lty=c(1:2), col=c(1,1,3,3))
plot(survfit(ss~tIdx+Arm, data=Cai2Clin), col=c(5,5,1,1),main="KRASwt CMS2/3 vs all other", lwd=2,
...
## Survival: Compare in cetux arm group CMS2/3 BRAF wt vs all
summary(coxph(ss~tIdx+Sex+Age, data=Cai2Clin, subset=(Arm=="wCetux")))
## Survival: Compare in BRAS/KRAS wt group CMS2/3 vs all
summary(coxph(ss~tIdx+Sex+Age, data=Cai2Clin, subset=(BRAF_KRAS==0)))
@
...
%\subsection{standardisation of data}
<<eval=F, echo=F>>==
library(ggplot2)

tempdat=data.frame(rbind(cbind(AMC_raw[, grep("total.norm", colnames(AMC_raw))], Samp="AMC"), 
                         cbind(Lei_range[, grep("total.norm", colnames(Lei_range))], Samp="Lei"),
                         cbind(Cai1_range[, grep("total.norm", colnames(Cai1_range))], Samp="Cai1"), 
                         cbind(Cai2_range[, grep("total.norm", colnames(Cai2_range))], Samp="Cai2")))
qplot(Samp,as.numeric(as.character(CDX2_Brown.total.norm)), data=tempdat)+geom_boxplot()

tempdat=data.frame(rbind(cbind(AMC_raw[, grep("StainInt", colnames(AMC_raw))], Samp="AMC"), 
                         cbind(Lei_range[, grep("StainInt", colnames(Lei_range))], Samp="Lei"),
                         cbind(Cai1_range[, grep("StainInt", colnames(Cai1_range))], Samp="Cai1"), 
                         cbind(Cai2_range[, grep("StainInt", colnames(Cai2_range))], Samp="Cai2")))
qplot(Samp,as.numeric(as.character(CDX2_StainInt)), data=tempdat)+geom_boxplot()

tempdat=data.frame(rbind(cbind(AMC_raw[, grep("StAreaFrac", colnames(AMC_raw))], Samp="AMC"), 
                         cbind(Lei_range[, grep("StAreaFrac", colnames(Lei_range))], Samp="Lei"),
                         cbind(Cai1_range[, grep("StAreaFrac", colnames(Cai1_range))], Samp="Cai1"), 
                         cbind(Cai2_range[, grep("StAreaFrac", colnames(Cai2_range))], Samp="Cai2")))
qplot(Samp,as.numeric(as.character(CDX2_StAreaFrac)), data=tempdat)+geom_boxplot()


## raw samples
AMC_raw=AMCdata.train$data[, -grep("Im", colnames(AMCdata.train$data))]
tempdat=data.frame(rbind(cbind(AMC_raw[, grep("total.norm", colnames(AMC_raw))], Samp="AMC"), 
                         cbind(Lei_raw[, grep("total.norm", colnames(Lei_raw))], Samp="Lei"),
                         cbind(Cai1_raw[, grep("total.norm", colnames(Cai1_raw))], Samp="Cai1"), 
                         cbind(Cai2_raw[, grep("total.norm", colnames(Cai2_raw))], Samp="Cai2")))
qplot(Samp,as.numeric(as.character(CDX2_Brown.total.norm)), data=tempdat)+geom_boxplot()


tempdat=data.frame(rbind(cbind(AMC_raw[, grep("StainInt", colnames(AMC_raw))], Samp="AMC"), 
                         cbind(Lei_raw[, grep("StainInt", colnames(Lei_raw))], Samp="Lei"),
                         cbind(Cai1_raw[, grep("StainInt", colnames(Cai1_raw))], Samp="Cai1"), 
                         cbind(Cai2_raw[, grep("StainInt", colnames(Cai2_raw))], Samp="Cai2")))
qplot(Samp,as.numeric(as.character(CDX2_StainInt)), data=tempdat)+geom_boxplot()

tempdat=data.frame(rbind(cbind(AMC_raw[, grep("StAreaFrac", colnames(AMC_raw))], Samp="AMC"), 
                         cbind(Lei_raw[, grep("StAreaFrac", colnames(Lei_raw))], Samp="Lei"),
                         cbind(Cai1_raw[, grep("StAreaFrac", colnames(Cai1_raw))], Samp="Cai1"), 
                         cbind(Cai2_raw[, grep("StAreaFrac", colnames(Cai2_raw))], Samp="Cai2")))
qplot(Samp,as.numeric(as.character(CDX2_StAreaFrac)), data=tempdat)+geom_boxplot()
